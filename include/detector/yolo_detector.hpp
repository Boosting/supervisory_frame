...
protected:
    ...
    vector<Target> get_detections(const image &im, int num, float thresh, box *boxes, float **probs, int classes);
    vector<Target> kitti_detect(const image &im, const network &net);
};
